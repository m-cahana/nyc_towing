...
plates_to_tow_2024 = plates_to_tow_2024.merge(
    tow_cases[tow_cases.boot_date.dt.year == 2024].drop_duplicates(subset=['plate_id', 'license_plate_issuing_state', 'license_plate_type']), 
    left_on=['plate', 'state', 'license_type'], 
    right_on=['plate_id', 'license_plate_issuing_state', 'license_plate_type'], 
    how='left'
)
...
print(f"""
      {plates_to_tow_2024.shape[0]} plates in judgement
      """)

print(f"""
      {plates_to_tow_2024[plates_to_tow_2024.case_number.isna()].shape[0]} plates never towed
      """)

print(f"""
      average fines in judgement among these plates:
      {plates_to_tow_2024[plates_to_tow_2024.case_number.isna()].fines_in_judgement.mean()}
      """)
